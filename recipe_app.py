...
class RecipeApp:
    def __init__(self):
        ...
        self.ingredient_tree = ttk.Treeview(
            right_frame,
        ...
        scrollbar = ttk.Scrollbar(
            ingredient_frame, orient=tk.VERTICAL, command=self.ingredient_tree.yview
        )
    ...
    def load_categories(self):
        ...
        with self.conn.cursor() as cursor:
            ...
            # 修改后
            categories = [("全部(0)", 0)] + [
                (row["type_name"] + "(" + str(row["id"]) + ")", row["id"])
    ...
    def show_ingredients(self, event):
        ...
        for ingredient in ingredients:
            self.ingredient_tree.insert(
                ...
                tags=(
                    ("lack",)
                    if ingredient["quantity"] < ingredient["quantity_required"]
                    else ()
                ),
            )
        ...
        try:
            with self.conn.cursor() as cursor:
                ...
                for item in cursor.fetchall():
                    self.ingredient_tree.insert(
                        "", "end", values=(item["name"], item["quantity"])
                    )
        except pymysql.Error as e:
